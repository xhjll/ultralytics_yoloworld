    ...
    def train(
        ...
        **kwargs,
        ...
        if hasattr(self.session, "model") and self.session.model.id:  # Ultralytics HUB session with loaded model
            if any(kwargs):
                LOGGER.warning("WARNING 鈿狅笍 using HUB training arguments, ignoring local training arguments.")
    ...
    def _smart_load(self, key: str):
        ...
        try:
            ...
            raise NotImplementedError(
                emojis(f"WARNING 鈿狅笍 '{name}' model does not support '{mode}' mode for '{self.task}' task yet.")
            ) from e
